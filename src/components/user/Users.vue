 ...
 * @LastEditTime: 2020-08-31 15:49:08
    ...
    <el-card>
      ...
      <el-table :data="userList" stripe>
        ...
        <el-table-column label="状态">
          <!-- 定义了一个作用域插槽:1.slot-scope接收当前作用域的数据；2.scope.row拿到对应的数据；3.v-model绑定具体的属性值 -->
...
export default {
    ...
    return {
      ...
      addFormRules: {
        username: [
          ...
          { min: 3, max: 10, message: '用户名的长度在3~10之间', triggle: 'blur' }
        ],
        password: [
          { required: true, message: '请输入密码', triggle: 'blur' },
          { min: 6, max: 15, message: '密码的长度在6~15之间', triggle: 'blur' }
        ],
        email: [
  ...
  methods: {
    ...
    // 监听添加用户对话框的关闭事件(注意:resetFields()是elementui表单的方法)
    addDialogClosed () {
      this.$refs.addFormRef.resetFields()
    },
    // 点击确定,添加新用户(注意:validate()是elementui表单的方法)
    addUser () {
